...
typedef long __attribute__((hyperobject)) *long_hp;
typedef long __attribute__((hyperobject)) long_h;
extern int x __attribute__((hyperobject));
// CHECK_LABEL: extern1
void extern1()
{
...
double local_reducer_1()
{
  ...
  double x __attribute__((hyperobject, reducer(reduce, identity))) = 0.0;
  // CHECK: call i8* @llvm.hyper.lookup
  // CHECK: load double
  // CHECK: fadd double
  // CHECK: store double
